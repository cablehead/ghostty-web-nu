...
const P=5083,B=`http://127.0.0.1:${P}`,ST="/tmp/xs-w"; rmSync(ST,{recursive:true,force:true});
...
const snap = ()=>pg.evaluate(()=>{
  ...
  return { width: p.style.width || 'full', paneW: Math.round(p.getBoundingClientRect().width), cols: g?.dataset.cols };
});
...
for(let i=0;i<4;i++){
  ...
  console.log("after Alt+O #"+(i+1)+":", JSON.stringify(await snap()));
}
